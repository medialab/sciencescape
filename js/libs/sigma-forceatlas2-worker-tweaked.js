    ...
    function pass() {
      ...
      if (W.settings.barnesHutOptimize) {
        ...
        for (n1 = 0; n1 < W.nodesLength; n1 += W.ppn) {
          for (n2 = 0; n2 < n1; n2 += W.ppn) {
            ...
            if(NodeMatrix[np(n1, 'dlevel')] == 0 && NodeMatrix[np(n2, 'dlevel')] == 0){
      ...
      for (e = 0; e < W.edgesLength; e += W.ppe) {
        ...
        if (distance > 0) {
          ...
          // Updating nodes' dx and dy
          if(NodeMatrix[np(n1, 'dlevel')] == NodeMatrix[np(n2, 'dlevel')] + 1 || NodeMatrix[np(n1, 'dlevel')] == NodeMatrix[np(n2, 'dlevel')]){
            NodeMatrix[np(n1, 'dx')] += xDist * factor;
            NodeMatrix[np(n1, 'dy')] += yDist * factor;
          }

          if(NodeMatrix[np(n2, 'dlevel')] == NodeMatrix[np(n1, 'dlevel')] + 1 || NodeMatrix[np(n1, 'dlevel')] == NodeMatrix[np(n2, 'dlevel')]){
            NodeMatrix[np(n2, 'dx')] -= xDist * factor;
            NodeMatrix[np(n2, 'dy')] -= yDist * factor;
          }
        }
